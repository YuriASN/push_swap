...
/*   Updated: 2022/11/14 20:33:07 by ysantos-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//printf("%sinto order A\n%s", YEL, CRESET);
	while (stack->nxt && (get_lowest(stack->nxt) > 0 || !check_order(stack)))
	{
		if (stack->nxt->value > stack->nxt->nxt->value)
			swap_stk(stack, 1);
		else if (get_lowest(stack->nxt) <= stksize(stack->nxt) / 2)
			rotate_stk(stack, 1);
		else
			rotate_rev(stack, 1);
...
//printf("%s\tinto order B\n%s", YEL, CRESET);
	while (!check_order_r(stack))
	{
		if (stack->nxt->value < stack->nxt->nxt->value)
			swap_stk(stack, 2);
		else if (stack->nxt->value < (stklast(stack->nxt))->value)
			rotate_rev(stack, 2);
		else
			rotate_stk(stack, 2);
	}
...
//getchar();
		...
		order1(stk_a, stk_b);
		if (stksize(stk_a->nxt) < 5 && !check_order(stk_a))
			order_a(stk_a);
